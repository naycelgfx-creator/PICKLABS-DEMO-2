...
const ESPN_SPORT_MAP: Record<string, { sport: string, league: string, groups?: string }> = {
    ...
    NCAAW: { sport: 'basketball', league: 'womens-college-basketball', groups: '50' },
    CFB: { sport: 'football', league: 'college-football', groups: '80' },
    ...
    const fetchTeams = useCallback(async (sport: string) => {
        ...
        try {
            if (sport === 'WBC') {
                ...
                // For NCAAB/NCAAW/CFB we still need the old endpoint due to groups param
                // Otherwise use the cleaner fetchAllTeams function
                if (map.groups) {
                    const url = `https://site.api.espn.com/apis/site/v2/sports/${map.sport}/${map.league}/teams?limit=150&groups=${map.groups}`;
                    const res = await fetch(url);
                    if (!res.ok) throw new Error(`ESPN returned ${res.status}`);
                    data = await res.json();
                } else {
                    const rawTeams = await fetchAllTeams(map.sport, map.league);
                    // Map it to object wrapper so parseESPNTeams handles it
                    data = { sports: [{ leagues: [{ teams: rawTeams.map(t => ({ team: t })) }] }] };
                }
