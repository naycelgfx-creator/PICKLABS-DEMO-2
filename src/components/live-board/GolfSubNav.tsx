...
import { SportKey, GOLF_LEAGUES } from '../../data/espnScoreboard';

interface GolfSubNavProps {
    activeLeague: SportKey;
    onSelectLeague: (league: SportKey) => void;
}

export const GolfSubNav: React.FC<GolfSubNavProps> = ({ activeLeague, onSelectLeague }) => {
    return (
        <div className="sticky top-[56px] z-20 bg-neutral-950/95 backdrop-blur border-b border-border-muted">
            <div className="max-w-[1536px] mx-auto px-6">
                <div className="flex items-center gap-1 overflow-x-auto scrollbar-none py-2">
                    <span className="text-[10px] text-slate-600 font-bold uppercase tracking-widest shrink-0 mr-2">League</span>

                    {GOLF_LEAGUES.map(league => {
