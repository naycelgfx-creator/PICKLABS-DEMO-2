...
function tennisMatchToGame(match: TennisMatch, sportKey: SportKey): Game {
    return {
        ...
        matchupId: match.id,
        awayTeam: { name: match.player2.displayName, abbr: match.player2.shortName || match.player2.displayName.split(' ').pop() || 'P2', logo: match.player2.flagUrl, score: match.player2.sets.reduce((a, b) => a + b, 0), winProb: 50 },
        homeTeam: { name: match.player1.displayName, abbr: match.player1.shortName || match.player1.displayName.split(' ').pop() || 'P1', logo: match.player1.flagUrl, score: match.player1.sets.reduce((a, b) => a + b, 0), winProb: 50 },
...
export const TennisTournamentPanel: React.FC<TennisTournamentPanelProps> = ({ sportKey, selectedDate, onSelectGame }) => {
    ...
    useEffect(() => {
        fetchMatches();
        // Auto-refresh every 12 hours (tennis matches can be long)
        const interval = setInterval(fetchMatches, 43200000); // 12 hours
