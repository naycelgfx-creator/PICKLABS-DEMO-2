        ...
        <motion.svg viewBox="0 0 48 48" fill="none" className={cn('', className)} style={{ width: size, height: size }}>
            <motion.g animate={{ rotate: 360 }} transition={{ duration: 12, ease: 'linear', repeat: Infinity }}>
                {[0, 45, 90, 135, 180, 225, 270, 315].map((deg) => (
                    <motion.line key={deg} x1="24" y1="6" x2="24" y2="10"
...
export function MoonIcon({ size = 48, className }: WeatherIconProps) {
    ...
    return (
        <svg viewBox="0 0 48 48" fill="none" className={cn('', className)} style={{ width: size, height: size }}>
            <path d="M28 8a14 14 0 100 28 10 10 0 010-28z" fill="#A78BFA" opacity={0.15} />
            <path d="M28 8a14 14 0 100 28 10 10 0 010-28z" stroke="#A78BFA" strokeWidth={2} strokeLinecap="round" />
            {stars.map((s) => (
                <motion.circle key={s.cx + '' + s.cy} cx={s.cx} cy={s.cy} r="1.2" fill="#A78BFA"
    ...
    return (
        <svg viewBox="0 0 48 48" fill="none" className={cn('', className)} style={{ width: size, height: size }}>
            <motion.g animate={{ x: [0, 2, 0, -2, 0] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}>
                ...
                <path d="M36 30H14a8 8 0 01-.5-16A10 10 0 0134 16a7 7 0 012 14z"
...
export function RainIcon({ size = 48, className }: WeatherIconProps) {
    ...
    return (
        <svg viewBox="0 0 48 48" fill="none" className={cn('', className)} style={{ width: size, height: size }}>
            ...
            {drops.map((drop) => (
                <motion.line key={drop.x} x1={drop.x} y1={26} x2={drop.x} y2={30}
...
export function HeavyRainIcon({ size = 48, className }: WeatherIconProps) {
    ...
    return (
        <svg viewBox="0 0 48 48" fill="none" className={cn('', className)} style={{ width: size, height: size }}>
            ...
            {drops.map((drop) => (
                <motion.line key={drop.x} x1={drop.x} y1={24} x2={drop.x - 2} y2={30}
        ...
        <svg viewBox="0 0 48 48" fill="none" className={cn('', className)} style={{ width: size, height: size }}>
            <motion.g animate={{ rotate: 360 }} transition={{ duration: 20, ease: 'linear', repeat: Infinity }}
                ...
                {[0, 60, 120, 180, 240, 300].map((deg) => (
                    <line key={deg} x1="16" y1="6" x2="16" y2="9"
        ...
        <svg viewBox="0 0 48 48" fill="none" className={cn('', className)} style={{ width: size, height: size }}>
            ...
            <motion.g animate={{ y: [4, 0] }} transition={{ duration: 3, repeat: Infinity, repeatType: 'reverse', ease: 'easeInOut' }}>
                ...
                {[0, 30, 60, 90, 120, 150, 180].map((deg) => (
                    <line key={deg} x1="24" y1="14" x2="24" y2="17"
                        stroke="#FBBF24" strokeWidth={1.5} strokeLinecap="round"
...
/* ─── Condition → Icon mapping (used by WeatherImpact) ─── */
export type WeatherCondition =
    | 'Sunny' | 'PartlyCloudy' | 'Cloudy' | 'Rain' | 'HeavyRain'
    | 'Snow' | 'Thunder' | 'Wind' | 'Fog' | 'Sunrise' | 'ClearNight' | 'Rainbow';
...
export function WeatherConditionIcon({ condition, size = 56, className }: WeatherIconMapProps) {
    const props = { size, className };
    switch (condition) {
        case 'Sunny': return <SunIcon {...props} />;
        case 'PartlyCloudy': return <PartlyCloudyIcon {...props} />;
        case 'Cloudy': return <CloudIcon {...props} />;
        case 'Rain': return <RainIcon {...props} />;
        case 'HeavyRain': return <HeavyRainIcon {...props} />;
        case 'Snow': return <SnowIcon {...props} />;
        case 'Thunder': return <ThunderIcon {...props} />;
        case 'Wind': return <WindIcon {...props} />;
        case 'Fog': return <FogIcon {...props} />;
        case 'Sunrise': return <SunriseIcon {...props} />;
        case 'ClearNight': return <MoonIcon {...props} />;
